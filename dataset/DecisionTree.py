...
def calc_var(data_framef,counterf,err_choice):
	...
	np_array=data_framef['output'].values
...
def split(data_frame,tree,min_leaf_size,err_choice):
	...
	a=data_frame.shape
	if a[0]<min_leaf_size or split_col=='output':
		#print('leaf node found')
		np_array=data_frame['output'].values
...
def main():
	...
	min_leaf_para=[2,3,4,5,6,7,8,9,10,15,20,30,35,40,45,50,60,80,100]
	err_method='abs'
	tree=Tree()
	x_coordinate=[]
	error_plot=[]
	...
	plt.title('Error using abs on Dataset 1')
	plt.show()	
